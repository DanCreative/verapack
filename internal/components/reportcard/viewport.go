...
// New returns a new Viewport with the given width and height as well as default
// key mappings.
func New(width, height int) (m Viewport) {
	m.Width = width
	m.Height = height
	m.setInitialValues()
	return m
...
// Viewport is the Bubble Tea Viewport for this viewport element.
...
// Init exists to satisfy the tea.Viewport interface for composability purposes.
...
func (m Viewport) VisibleLineCount() int {
...
// number of lines. Use Viewport.ViewDown to get the lines that should be rendered.
// For example:
//
//	lines := Viewport.ViewDown(1)
...
// number of lines height. Use Viewport.ViewUp to get the lines that should be
// rendered.
...
func (m Viewport) Update(msg tea.Msg) (Viewport, tea.Cmd) {
	var cmd tea.Cmd
	m, cmd = m.updateAsViewport(msg)
	return m, cmd
}

// Author's note: this method has been broken out to make it easier to
// potentially transition Update to satisfy tea.Viewport.
func (m Viewport) updateAsViewport(msg tea.Msg) (Viewport, tea.Cmd) {
	if !m.initialized {
		m.setInitialValues()
	}
